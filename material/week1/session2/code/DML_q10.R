...
## read data from csv
data <- as.data.frame(fread(file = "data/Synthetic4BookChap.csv"))
years <- 2003:2007
## subset data
ix <- data$year %in% years
Dt <- data[ix, c(inputs, "TA")]
Dt$Y <- log(data[ix, "RECO_syn"]) + rnorm(n = sum(ix), sd = 0.01)
...
model$tune(
  list("ml_m" = ps("max.depth" = p_int(lower = 1, upper = 10)),
       "ml_l" = ps("max.depth" = p_int(lower = 1, upper = 10))),
